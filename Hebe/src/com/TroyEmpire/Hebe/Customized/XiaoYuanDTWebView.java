...
public class XiaoYuanDTWebView extends WebView {
	...
	public void initiate(int campusId, Activity activity) {
		...
		buildingService = new BuildingService(campusId, activity);
	...
	public void disPlayShortestPath(long sourceId, long destId) {
		...
		for (int i = 0; i < res.length; i++) {
			PathDot pathDot = mapService.getPathDot(Integer.parseInt(res[i]));
			if (pathDot != null) {
		...
		public void callAndroid(final String arg) throws InterruptedException { // must
			...
			Thread child = new Thread() {
				public void run() {
					...
					//building.getId() == 1 就是什么都没返回
					if( (isInAddMarkerForPathMode == 0) && (building.getId() != 0) ) addMarkerForSearch(building);
				}
			};
			child.start();
		//	child.join();
