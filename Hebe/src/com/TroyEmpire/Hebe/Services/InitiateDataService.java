...
import com.TroyEmpire.Hebe.Util.HebeFileHandleToolUtil;
...
public class InitiateDataService implements IInitiateDataService {
	...
	public boolean initiateRestaurantData(int campusId) {
		...
		String absolutePath = Environment.getExternalStorageDirectory()
				.getAbsolutePath();
		final String sourceUrl = Constant.HEBE_SERVER_URL
				+ "/downloadData/restaurants/" + campusId;
		final String destFolderPath = absolutePath + "/" + Constant.RESTAURANT_DATA_PATH;
		...
		try {
			child.join();
			if (HebeFileHandleToolUtil.unzipFile(destFolderPath + "/temp.zip",
	...
	public boolean initiateMapData(int campusId) {
		...
		final String destFolderPath = absolutePath + "/" + Constant.MAP_DATA_PATH;
		Thread child = new Thread() {
			public void run() {
				downloadZipData(sourceUrl, destFolderPath);
			}
